...
export default class RoleRepository {
    ...
    public GetAllRoles = async (): Promise<RoleDto[]> => this.role.find()
    .populate("permissions");

    public GetRole = async (_id: string): Promise<RoleDto> => this.role.findOne({_id}).populate("permissions");

    public GetRoleByName = async (name: string): Promise<RoleDto> => (this.role.findOne({name}).populate("permissions"));

    public CreateRole = async (role: Partial<RoleDto>): Promise<RoleDto> => {
        try{
            return await (await this.role.create(role)).toObject({ useProjection: true });
        }catch(e){
